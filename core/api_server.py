...
def post_chat(req: ChatRequest):
    """
    Structured tactical chat endpoint.
    Routes through ChatEngine → Controller with health/context awareness.
    """
    if not nova_app:
        return {"error": "App not initialized"}

    if not chat_engine:
        return {"error": "ChatEngine not initialized"}

    # Build current metrics (same as /api/summary)
    is_alive = (daemon_thread is not None and daemon_thread.is_alive())
    last_error = daemon.last_error if daemon else None
    ...
    current_health_data = {}
    if health_engine:
        current_health_data = health_engine.calculate_health(current_metrics)

    # Route through ChatEngine
    return chat_engine.handle_message(
        message=req.message,
        session_id=req.session_id,
        current_health_data=current_health_data,
