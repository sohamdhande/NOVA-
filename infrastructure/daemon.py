...
class NovaDaemon:
    def __init__(self, memory_tool, notion_tool, pdf_tool, event_loop=None):
        self.running = False
        self.last_error = None
        self.start_time = None
        self._loop = event_loop  # shared asyncio loop from main.py
    ...
    def run(self, handle_signals=True):
        ...
        # Use the shared event loop passed from main.py
        loop = self._loop
        if loop is None:
            print("[NOVA] ⚠ Daemon has no shared event loop — event publishing disabled.")
        
        # Publish daemon started (event_bus is already started by main.py)
        if loop:
        ...
        scan_interval = 300  # Reduced background load (was 10)
                    ...
                    if os.path.exists("nova_settings.json"):
                        try:
                            with open("nova_settings.json") as f:
                                scan_interval = json.load(f).get("interval_telemetry", 300)
                        except Exception:
                            pass
